...
class Reinforce(object):
    ...
    def train(self, num_episodes):
        ...
        for e in range(num_episodes):
            ...
            if (e+1) % (self.save_freq) == 0:
                torch.save(self.policy.state_dict(), os.path.join(self.save_path, "{}_{}.pkl".format(self.expt_name,e)))
    ...
    def plot_rewards(self, save=False):
        ...
        if save :
            plt.savefig(self.expt_name + "_train_rewards_{}.png".format(len(self.train_rewards)))
        else:
            plt.show()
        ...
        try:
            ...
            ipdb.set_trace()
        if save :
            plt.savefig(self.expt_name + "_test_rewards_{}.png".format(len(self.test_rewards)))
        else:
            plt.show()
        ...
        if save :
            plt.savefig(self.expt_name + "_train_steps_{}.png".format(len(self.train_steps)))
        else:
            plt.show()
            # pause so that the plots are updated
            plt.pause(0.001)
        ...
        if save :
            plt.savefig(self.expt_name + "_test_steps_{}.png".format(len(self.test_steps)))
        else:
            plt.show()
        ...
        plt.figure(5)
        plt.clf()
        plt.title("Avg. Training Loss over {} episodes".format(len(train_nepisodes)))
        plt.xlabel("Number of training episodes")
        plt.ylabel("Avg. Loss")
        plt.plot(self.losses, color="crimson")
        if save :
            plt.savefig(self.expt_name + "_train_loss_{}.png".format(len(self.test_rewards)))
        else:
            plt.show()
...
def main():
    ...
    parser.add_argument("--test_episodes", type=int, default=10, help="number of episodes to test on")
    parser.add_argument("--num_steps", type=int, default=50, help="number of steps to run per episode")
    parser.add_argument("--seed", type=int, default=1234, help="random seed")
    parser.add_argument("--save_freq", type=int, default=1e5, help="checkpoint frequency for saving models")
    parser.add_argument("--test_freq", type=int, default=200, help="frequency for testing policy")
    parser.add_argument("--save_path", type=str, default="models", help="path for saving models")
    parser.add_argument("--expt_name", type=str, default="plots/reinforce1", help="expt name for saving results")
