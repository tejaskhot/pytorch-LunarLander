...
class Imitation():
    def __init__(self, model_config_path, expert_weights_path):
        ...
        #VERIFY: SPECIFY THE LEARNING RATE
    ...
    def train(self, env, num_episodes=100, num_epochs=50, render=False):
        ...
        metric = self.model.fit(x_train, y_train_cat, epochs=num_epochs, shuffle=True, verbose=0) #,batch_size=1)
...
def main(args):
    ...
    ###states,actions,rewards = self.run_model(env,render)
    ###x_test = np.asarray(states)
    ###y_test = np.asarray(actions)
